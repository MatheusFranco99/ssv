...
func TestAleaVCBCSend(t *testing.T) {
...
// 	// VCBC from 1
	...
	// agg_msg_1, err := AggregateMsgs([]*messages.SignedMessage{vcbc_ready_message_1_1, vcbc_ready_message_1_2, vcbc_ready_message_1_3})
	// if err != nil {
	// 	panic(err)
	// }
	// vcbc_final_message_1 := VCBCFinalMessage(1, TestingHeight, 1, agg_msg_1, TestingContentHash, inst.State)
...
func BenchmarkAleaVCBCReady(t *testing.B) {
...
// 	// VCBC from 1
	...
	for i := 0; i < t.N; i++ {
		...
		// t.Logf("First-time execution took %s", elapsed)
			_, _, _, err = inst.ProcessMsg(vcbc_ready_message_1_1)
			require.NoError(t, err)
			// _, _, _, err = inst.ProcessMsg(vcbc_ready_message_1_2)
			// require.NoError(t, err)
